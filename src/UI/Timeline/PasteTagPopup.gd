...
onready var instructions: Label = $PanelContainer/VBoxContainer/Instructions
...
func _ready() -> void:
	...
	from_project_list.connect("item_selected", self, "_on_FromProject_changed")
	animation_tags_list.connect("item_selected", self, "_on_TagList_id_pressed")
...
func refresh_list() -> void:
	for tag in from_project.animation_tags:
		...
		if title == "":
			title = "(Untitled)"
		animation_tags_list.add_item(title, tex)
...
func _on_TagContainer_gui_input(event: InputEvent) -> void:
	...
	if Input.is_action_just_released("right_mouse"):
		...
		instructions.text = str("Paste content from tag:")
...
func add_animation(indices: Array, destination: int):
	...
	var from_l_names := PoolStringArray()
	for l in from_project.layers:
		from_l_names.append(l.name)
	var to_l_names := PoolStringArray()
	for l in project.layers:
		to_l_names.append(l.name)
	var targets := PoolIntArray()  # target layers the content will be copied to
	var search_idx = 0
	...
	for f in indices:
			...
			if l in targets:
				...
				if src_cel is Cel3D:
					...
					if src_cel is PixelCel:
						...
						var copy := Image.new()
						copy.create(project.size.x, project.size.y, false, Image.FORMAT_RGBA8)
		...
		for tag in new_animation_tags:  # Loop through the tags to see if the frame is in one
			if copied_indices[0] >= tag.from && copied_indices[0] <= tag.to:
				...
				tag.from += 1
				tag.to += 1
	project.undo_redo.add_do_method(project, "add_layers", added_layers, added_idx, added_cels)
	...
	project.undo_redo.add_undo_method(project, "remove_layers", added_idx)
