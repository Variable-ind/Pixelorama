...
func open_image_as_spritesheet_layer(
	_path: String, image: Image, file_name: String, horizontal: int, vertical: int, start_frame: int, position: Vector2, velocity: Vector2, acceleration: Vector2, repeat_times :int = 0
) -> void:
	...
	var new_frames_size = (start_frame + ((vertical * horizontal) * (repeat_times + 1)))
	...
	for _time in range(repeat_times + 1):
		for yy in range(vertical):
			for xx in range(horizontal):
				...
				if frame_index > start_frame:
					...
					position += velocity
				cropped_image.convert(Image.FORMAT_RGBA8)
				var new_image: Image = new_frames[frame_index].cels[layer_index].image
				var rect: Rect2 = Rect2(Vector2.ZERO, cropped_image.get_size())
				new_image.blit_rect(cropped_image, rect, position)
				new_frames[frame_index].cels[layer_index] = (Cel.new(new_image, 1))
				image_no += 1
